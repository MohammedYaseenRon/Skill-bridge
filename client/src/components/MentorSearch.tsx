...
import { Star, MapPin, Clock, DollarSign, User, Filter } from 'lucide-react';
          ...
          {recommendations.map((mentor) => (
            <Card key={mentor.mentor_id} className="hover:shadow-lg transition-shadow">
                ...
                <div className="flex items-start justify-between">
                  <div className="flex items-center space-x-3">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-gray-600">{mentor.mentor_job_title}</p>
                      {mentor.mentor_company && (
                        <p className="text-xs text-gray-500">{mentor.mentor_company}</p>
                      )}
                    </div>
                  </div>
                  <Badge variant={getScoreBadgeVariant(mentor.recommendation_score.total_score)}>
              ...
              <CardContent className="space-y-4">
                {/* Bio */}
                {mentor.mentor_bio && (
                  <p className="text-sm text-gray-700 line-clamp-3">{mentor.mentor_bio}</p>
                )}
                ...
                {mentor.mentor_hourly_rate !== undefined && (
                  <div className="flex items-center space-x-1 text-sm">
                    ...
                    <span className="font-medium">
                      {mentor.mentor_hourly_rate === 0 ? 'Free' : `$${mentor.mentor_hourly_rate}/hour`}
                    </span>
                  </div>
                )}
                ...
                <div className="pt-4 border-t">
                  <div className="grid grid-cols-2 gap-2 text-xs">
                    <div className="flex justify-between">
                      <span>Skills:</span>
                      <span className={getScoreColor(mentor.recommendation_score.skills_match)}>
                        {mentor.recommendation_score.skills_match.toFixed(0)}%
                      </span>
                    </div>
                    <div className="flex justify-between">
                      <span>Location:</span>
                      <span className={getScoreColor(mentor.recommendation_score.location_match)}>
                        {mentor.recommendation_score.location_match.toFixed(0)}%
                      </span>
                    </div>
                    <div className="flex justify-between">
                      <span>Language:</span>
                      <span className={getScoreColor(mentor.recommendation_score.language_match)}>
                        {mentor.recommendation_score.language_match.toFixed(0)}%
                      </span>
                    </div>
                    <div className="flex justify-between">
                      <span>Experience:</span>
                      <span className={getScoreColor(mentor.recommendation_score.experience_match)}>
                        {mentor.recommendation_score.experience_match.toFixed(0)}%
                      </span>
                    </div>
                  </div>
                </div>
      ...
      {!isLoading && !error && recommendations.length === 0 && (
        <div className="text-center py-12">
          ...
          <p className="text-gray-600 mb-4">
            Try adjusting your filters or update your learning preferences in your profile.
          </p>
